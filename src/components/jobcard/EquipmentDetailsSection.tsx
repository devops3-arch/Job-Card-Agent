...
const EquipmentDetailsSection = ({ data, onChange }: Props) => {
  ...
  const handleEquipmentModelChange = (value: string) => {
    onChange({ ...data, equipmentModel: value });
    if (value.trim()) {
  ...
  const selectEquipment = (equipment: typeof equipmentMasterList[0]) => {
    onChange({
      ...
      equipmentBrandDescription: equipment.brand,
    });
    setShowSuggestions(false);
  };
    ...
    <div className="section-card">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-4">
        {fields.map((field, index) => {
          // Equipment Model field with autocomplete
            ...
            return (
              <motion.div key={field.field} custom={index} variants={fieldVariants} initial="hidden" animate="visible" className="relative">
                ...
                <Input
                  ...
                  onChange={(event) => handleEquipmentModelChange(event.target.value)}
                  onFocus={() => data.equipmentModel && setShowSuggestions(true)}
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 bg-background border border-border rounded-lg shadow-lg max-h-48 overflow-y-auto z-10">
                    {equipmentSuggestions.map((eq, idx) => (
                      ...
                      >
                        ...
                        <div className="text-xs text-muted-foreground">{eq.brand} • {eq.type}</div>
                      </div>
                    ))}
                  </div>
                )}
